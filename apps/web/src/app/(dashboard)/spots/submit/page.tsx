...
export default function SubmitSpotPage() {
  ...
  if (done) {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        ...
        <h2 className="text-xl font-bold text-[var(--foam)] mb-2">Spot submitted!</h2>
        <p className="text-[var(--spray)] text-sm mb-6">
          Thanks for contributing. Our team will review it and add it to the map within 48 hours.
        </p>
        <div className="flex gap-3 justify-center">
          <Link href="/map" className="btn-ocean">
            Back to map
          </Link>
          <button
            onClick={() => { setForm(EMPTY); setDone(false) }}
            className="bg-[var(--paper-sunken)] hover:bg-[var(--tile-border)] border border-[var(--tile-border-strong)] text-[var(--mist)] text-sm px-4 py-2 rounded-lg transition-colors"
          >
            Submit another
          </button>
        </div>
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6">
      <Link href="/map" className="text-[var(--spray)] hover:text-[var(--foam)] text-sm flex items-center gap-1 mb-6 transition-colors">
        ← Back to map
      </Link>

      <h1 className="text-2xl font-bold text-[var(--foam)] mb-1">Submit a Spot</h1>
      <p className="text-[var(--spray)] text-sm mb-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm capitalize transition-colors ${
                    form.break_type === bt
                      ...
                      : 'bg-[var(--paper-sunken)] border border-[var(--tile-border)] text-[var(--spray)] hover:text-[var(--foam)]'
                  }`}
                >
                  {bt}
                </button>
        ...
        <Card title="Location">
          ...
          <p className="text-xs text-[var(--deep-text)] mt-1">
            Find coordinates at{' '}
            <a href="https://www.google.com/maps" target="_blank" rel="noopener noreferrer" className="text-[var(--cyan-bright)] hover:underline">
              Google Maps
            </a>
        ...
        <Card title="Data Sources (optional)">
          <Field label="Nearest NDBC buoy ID">
            ...
            <p className="text-xs text-[var(--deep-text)] mt-1">
              Find at{' '}
              <a href="https://www.ndbc.noaa.gov" target="_blank" rel="noopener noreferrer" className="text-[var(--cyan-bright)]">
                ndbc.noaa.gov
              </a>
            </p>
          </Field>
...
function Card({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="tile rounded-2xl p-5 space-y-4">
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-xs text-[var(--spray)] mb-2">{label}</label>
      {children}
    </div>
  )
}
